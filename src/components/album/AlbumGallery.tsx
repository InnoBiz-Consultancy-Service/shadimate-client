...
// /* ── Mock blurred cards for non-premium ── */
...
//   photos: AlbumPhoto[];
//   userId: string; // ← required: যে user এর profile দেখা হচ্ছে তার ID
...
//   const { photos, loading, fetchUserAlbum } = useAlbum(initialPhotos);

//   useEffect(() => {
//     const premium = checkIsPremium();
//     setIsPremium(premium);

//     // Premium user হলে fresh data server থেকে fetch করো
//     if (premium && userId) {
//       fetchUserAlbum(userId);
//     }
//     // eslint-disable-next-line react-hooks/exhaustive-deps
//   }, [userId]);

//   // Loading skeleton — premium check হচ্ছে বা photos fetch হচ্ছে
...
//   // Non-premium: blurred mock cards দেখাও
//   if (!isPremium) {
//     return (
//       <>
//         <div className="grid grid-cols-3 gap-1.5">
...
interface AlbumGalleryProps {
  ...
  isOwnProfile?: boolean;
  /**
   * Server থেকে pass করা viewer এর premium status।
   * isOwnProfile true হলে এই prop এর দরকার নেই।
   */
...
export default function AlbumGallery({
  ...
  // Own profile হলে সবসময় দেখাবে, অন্যথায় viewer premium কিনা check করো
  const canViewPhotos = isOwnProfile || viewerIsPremium;

  // Non-premium viewer: blur দেখাও
  if (!canViewPhotos) {
    return (
      <>
        <div className="grid grid-cols-3 gap-1.5">
  ...
  // Premium / own profile + কোনো photo নেই
  if (photos.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-8 px-4 text-center">
        <div className="w-12 h-12 rounded-full bg-brand/10 flex items-center justify-center mb-2">
  ...
  // Premium / own profile: normal gallery
  const displayPhotos = viewAll ? photos : photos.slice(0, 6);
