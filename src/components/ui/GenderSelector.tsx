...
const ManSVG = ({ active }: { active: boolean }) => (
  <svg width="22" height="22" viewBox="0 0 28 28" fill="none">
    <circle
      ...
      cy="10"
      r="5"
      stroke={active ? "var(--color-brand)" : "var(--color-text-muted)"}
      strokeWidth="1.8"
    />
    <path
...
export default function GenderSelector({
  ...
  return (
    <div>
      <p
        className={`font-outfit text-white ${dt.inputLabel} mb-2.5 ${error ? "text-red-500" : ""}`}
      >
        Gender
      </p>
        ...
        {(["male", "female"] as const).map((g) => (
          <button
            ...
            style={{
              ...
              background:
                value !== g
                  ? "rgb(from var(--color-brand) r g b / 0.04)"
          ...
          >
            ...
            <span
              className={`text-sm font-semibold capitalize ${
                value !== g ? "text-white" : dt.textSecondary
              }`}
            >
              {g}
            </span>
      ...
      {error && (
        <p
          className={`${dt.inputError} text-xs mt-1.5 animate-[fadeIn_0.2s_ease]`}
        >
          {error}
        </p>
      )}
