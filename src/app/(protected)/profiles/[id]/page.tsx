...
/* ── Server-side JWT decode (no verification, only for subscription field) ── */
...
function getViewerIsPremiumFromToken(token: string | null): boolean {
...
async function ProfileContent({ id }: { id: string }) {
  ...
  // Viewer এর token থেকে premium status বের করো (server-side, reliable)
  const viewerIsPremium = getViewerIsPremiumFromToken(
    currentUserProfileStatus.token,
  );
  ...
  const [likeCountRes, albumRes, blockStatusRes, ignoreStatusRes] =
    await Promise.all([
      ...
      // URL param `id` is always the userId — ProfileCard uses /profiles/${userId}
      getUserAlbum(id).catch(() => ({
        success: false,
        data: undefined,
      })),
      isLoggedIn && !isOwnProfile
