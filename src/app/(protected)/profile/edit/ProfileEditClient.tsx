...
import {
  ...
  EDUCATION_INSTITUTION_LABEL,
  EDUCATION_USES_UNIVERSITY_API,
  FAITH_OPTIONS,
...
export default function ProfileEditClient({
  ...
  /* unified institution name — used when not using university API */
  ...
  const validateStep = (s: number): Record<string, string> => {
    ...
    if (s === 4) {
      ...
      const needsUniApi = EDUCATION_USES_UNIVERSITY_API.has(eduVariety);
      if (needsUniApi && !universityId && !customInstituteName.trim())
        errs.instituteName = "Please select or enter a university";
      if (!needsUniApi && !customInstituteName.trim())
        errs.instituteName = `Please enter the ${EDUCATION_INSTITUTION_LABEL[eduVariety] || "Institution Name"}`;
  ...
  const buildStepPayload = (s: number): Record<string, unknown> => {
    switch (s) {
      ...
      case 4: {
        const needsUniApi = EDUCATION_USES_UNIVERSITY_API.has(eduVariety);
        return eduVariety || universityId || department
          ? {
              education: {
                graduation: {
                  variety: eduVariety || undefined,
                  universityId: needsUniApi
                    ? universityId || undefined
                    : undefined,
                  department: department || undefined,
                  institution: !needsUniApi
                    ? customInstituteName || undefined
                    : undefined,
  ...
  const buildFullPayload = (): Record<string, unknown> => {
    ...
    for (let s = 1; s <= TOTAL_STEPS; s++) {
      Object.assign(p, buildStepPayload(s));
    }
    return p;
  };

  /* ── Save ── */
  const handleSave = async (stepOverride?: number) => {
    setSaving(true);
    ...
    if (res.success)
      setTimeout(() => router.push(isEdit ? "/profile" : "/feed"), 1500);
  };
  ...
  const renderPreview = () => (
    <div className="space-y-4">
      ...
      >
        ...
        <PreviewRow
          label={EDUCATION_INSTITUTION_LABEL[eduVariety] || "Institution"}
          value={universityName || customInstituteName}
        />
                ...
                <div className="flex flex-col gap-1.5">
                  {EDUCATION_USES_UNIVERSITY_API.has(eduVariety) ? (
                    ...
                    <Field
                      ...
                      onChange={(v) => {
                        setCustomInstituteName(v);
                        clearError("instituteName");
                      }}
                      placeholder={
                        eduVariety === "HSC"
                          ? "e.g. Dhaka City College"
                          : eduVariety === "SSC"
                            ? "e.g. Motijheel Govt. High School"
                            : "e.g. BTEC Institute, Dhaka"
                      }
                  ...
                  {errors.instituteName &&
                    EDUCATION_USES_UNIVERSITY_API.has(eduVariety) && (
                      <p className="flex items-center gap-1 text-[11px] text-red-500 font-outfit">
                        <AlertCircle size={11} /> {errors.instituteName}
                      </p>
                    )}
                </div>
              )}
