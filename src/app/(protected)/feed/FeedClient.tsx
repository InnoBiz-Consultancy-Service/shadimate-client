...
// FIX: Added "all" tab — "all" = same as latest but labelled differently per spec
type Tab = "all" | "latest" | "online";
...
export default function FeedClient({
  ...
  // FIX: Default active tab is "All"
  const [activeTab, setActiveTab] = useState<Tab>("all");

  // ── Latest/All tab state ──
  const [latestProfiles, setLatestProfiles] = useState(initialLatestProfiles);
  const [latestMeta, setLatestMeta] = useState(initialLatestMeta);
  const [latestFilters, setLatestFilters] = useState<ProfileFilters>({
    page: 1,
  ...
  const isPending = isOnline ? onlinePending : latestPending;
  const profiles = isOnline ? onlineProfiles : latestProfiles;
  const meta = isOnline ? onlineMeta : latestMeta;
  ...
  return (
    <div className="font-outfit min-h-screen px-3 py-4 md:py-8 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          {meta.total > 0 && (
            <span className="text-gray-400 text-xs">{meta.total} profiles</span>
          )}
        </div>
        <Link
          href="/profile/edit"
      ...
      {!hasProfile && (
        <div className="mb-4 rounded-2xl border border-amber-300 bg-amber-50 px-4 py-3.5 flex items-center gap-3">
          ...
          <Link
            href="/profile/edit"
            className="shrink-0 px-3 py-1.5 rounded-xl text-xs font-bold text-white bg-linear-to-r from-brand to-accent no-underline"
          >
            Create
          </Link>
        </div>
      ...
      <div className="flex items-center gap-1 mb-4 p-1 rounded-2xl bg-gray-100 w-fit">
        <TabButton
          ...
          label="All"
          count={activeTab === "all" ? meta.total : initialLatestMeta.total}
        />
        <TabButton
          active={activeTab === "latest"}
          onClick={() => handleTabChange("latest")}
      ...
      {!isPending && profiles.length > 0 && (
        <>
          ...
          <Pagination
            ...
            totalPages={meta.totalPages}
            onPageChange={
              isOnline ? handleOnlinePageChange : handleLatestPageChange
            }
          />
        </>
      )}
